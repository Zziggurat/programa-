...
| Simulación | `src/motores/simulacion.ts` | Energiza el tablero, converge la red y los contactos a punto fijo y, después, ejecuta los PLC por scans separados. Todos capturan una imagen congelada de entradas y publican salidas atómicamente: los enclavamientos eléctricos convergen sin adelantar temporizadores, contadores o flancos. Conserva corriente por fase, cortocircuitos funcionales, disparos estimados, motores/VFD y señales analógicas con calidad |
| Programa del controlador | `src/motores/plc-compilador.ts`, `src/motores/plc-runtime.ts` | DSL V4 compilado a IR tipada sin `eval`: BOOL/REAL, imágenes DI/DO/AI/AO, flancos, TON/TOF/TP, CTU/CTD, secuencias con prioridad, alarmas, interlocks, PID V1, watchdog, fuerzas y RUN/STOP/pausa/scan único. `src/motores/logica.ts` permanece como adaptador compatible para proyectos históricos |
