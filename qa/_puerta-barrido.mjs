/** La puerta a medio abrir, mirada desde arriba: es donde se vería si la hoja atraviesa el cuerpo. */
...
// Cámara cenital: mirando el armario desde arriba, la hoja y el cuerpo se ven como dos líneas.
await qa('verDesde', { x: 0, y: 1500, z: 120, tx: 0, ty: 0, tz: 60 });
await p.waitForTimeout(400);
for (const t of [0, 0.25, 0.5, 0.75, 1]) {
	await qa('ponerPuerta', t);
	await p.waitForTimeout(320);
	await p.screenshot({ path: join(SALIDA, `barrido-${String(Math.round(t * 100)).padStart(3, '0')}.png`) });
	console.log(`  puerta al ${Math.round(t * 100)} %`);
}
