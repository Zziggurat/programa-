...
async function abrirEjemplo(){
	...
	await page.locator('#modal-ejemplos').waitFor({state:'visible'});await page.locator('.tarjeta-ejemplo',{hasText:'Fixture V7: desbalance trifásico'}).first().getByRole('button',{name:/Abrir y estudiar/i}).click();
	await Promise.race([page.locator('#modal-dialogo').waitFor({state:'visible',timeout:1200}).catch(()=>false),page.waitForFunction(()=>window.qa.proyecto().nombre==='Fixture V7 — trifásico desbalanceado',null,{timeout:1200}).catch(()=>false)]);
	if(await page.locator('#modal-dialogo').isVisible().catch(()=>false))await click('dialogo-ok');
	await page.waitForFunction(()=>window.qa.proyecto().nombre==='Fixture V7 — trifásico desbalanceado',null,{timeout:30000});
...
try{
	...
	const base=await page.evaluate(()=>{const c=window.qa.proyecto().conductores.find(c=>c.id==='wl1');return{seccion:c.seccion,borne:c.de.dispositivoId==='red'?c.de.borneId:c.a.borneId}});
	await page.locator('[data-ing-scenario-conductor]').selectOption('wl1');await page.locator('[data-ing-scenario-section]').fill('4');await page.locator('[data-ing-scenario-run]').click();
	await page.locator('[data-ing-scenario-result="A"]').waitFor({state:'visible'});
	comprobar('A compara sección 2.5 → 4 y publica deltas',/BASE vs.*4 mm².*BASE NO MODIFICADA.*Deltas de conductores.*wl1/s.test(await page.locator('[data-ing-scenario-result="A"]').innerText()));
	comprobar('comparar A no muta BASE',await page.evaluate(()=>window.qa.proyecto().conductores.find(c=>c.id==='wl1').seccion)===base.seccion);

	await page.locator('[data-ing-scenario-slot]').selectOption('B');await page.locator('[data-ing-scenario-type]').selectOption('ASIGNACION_FASE');
	await page.locator('[data-ing-scenario-phase-conductor]').selectOption('wl1');await page.locator('[data-ing-scenario-source]').selectOption('red');await page.locator('[data-ing-scenario-phase]').selectOption('L2');await page.locator('[data-ing-scenario-run]').click();
	comprobar('BASE/A/B se muestran como dos overlays independientes',await page.locator('[data-ing-scenario-result]').count()===2);
	comprobar('comparar B L1 → L2 tampoco muta la fase BASE',await page.evaluate((borne)=>{const c=window.qa.proyecto().conductores.find(c=>c.id==='wl1');return(c.de.dispositivoId==='red'?c.de.borneId:c.a.borneId)===borne},base.borne));
	const textoB=await page.locator('[data-ing-scenario-result="B"]').innerText();
	comprobar('B expone deltas fasoriales de balance y neutro',/wl1.*L2/s.test(textoB)&&/Desequilibrio.*Neutro/s.test(textoB),textoB);
	...
	await page.waitForFunction(()=>window.qa.proyecto().conductores.find(c=>c.id==='wl1').seccion===4,null,{timeout:15000});
	await page.evaluate(()=>window.qa.esperarPersistencia());
	comprobar('aplicar A modifica sección pero no aplica B',await page.evaluate(()=>{const c=window.qa.proyecto().conductores.find(c=>c.id==='wl1');return c.seccion===4&&(c.de.dispositivoId==='red'?c.de.borneId:c.a.borneId)==='L1'}));
	await page.reload({waitUntil:'domcontentloaded'});await esperarEditorListo(page);
	comprobar('el escenario aplicado persiste al recargar sin aplicar B',await page.evaluate(()=>{const c=window.qa.proyecto().conductores.find(c=>c.id==='wl1');return c.seccion===4&&(c.de.dispositivoId==='red'?c.de.borneId:c.a.borneId)==='L1'}));
