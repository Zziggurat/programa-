...
export function crearSnapshotDisenoAsistido(entrada: { proyecto: Proyecto; solicitud: SolicitudDisenoAsistido;
	revisionesDisponibles?: readonly RevisionTecnica[]; contextoFisico?: SnapshotDisenoAsistido['contextoFisico'] }): SnapshotDisenoAsistido {
	...
	const hash = hashSnapshotTecnico({ version: 1, hashBase, solicitud, revisiones: revisiones.map(referenciaTecnica), contextoFisico: serializable(entrada.contextoFisico) });
	return { version: 1, hash, hashBase, proyecto, solicitud, revisiones, contextoFisico: entrada.contextoFisico ? structuredClone(entrada.contextoFisico) : undefined };
}
...
export function *generarPlanesDiseno(snapshot: SnapshotDisenoAsistido, espacio=construirEspacioOpciones(snapshot)): Generator<PlanDisenoAsistido> {
	...
	const protecciones=espacio.protecciones.map(referencia=>plan('PROTECCION',[{tipo:'PROTECCION',dispositivoId:s.proteccionId!,referencia}]));
	for (const p of secciones) yield p; for (const p of protecciones) yield p;
	for (const sec of secciones) for (const prot of protecciones) yield plan('COMBINADO',[...sec.cambios,...prot.cambios]);
}
...
function registrarDecision(proyecto:Proyecto,snapshot:SnapshotDisenoAsistido,planEntrada:PlanDisenoAsistido):void{
	const d:DecisionDisenoAsistidoPersistida={version:1,id:sha256Texto(jsonCanonico({snapshot:snapshot.hash,plan:planEntrada.id})),solicitudId:snapshot.solicitud.id,planId:planEntrada.id,hashBase:snapshot.hashBase,circuitoId:snapshot.solicitud.circuitoId,cambios:canonCambios(planEntrada.cambios).map(c=>c.tipo==='SECCION'?c:{...c,referencia:{...c.referencia,tipo:'PRODUCTO' as const}})};
