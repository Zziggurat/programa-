...
Persistir objetos `{x,y,z}` para cada muestra repetiría miles de claves JSON en los ejemplos grandes. La V4 usa series numéricas planas sin cuantización para conservar XYZ exacto y reducir ese coste. La medición del documento denso está abajo; sigue pendiente medir el coste acumulado de snapshots y autosave multiproyecto antes de activar la asignación por defecto en la interfaz.
...
`qa/localidad-densa-m6.mjs` importa el ejemplo estrella-triángulo: contiene 61 conexiones, de las cuales 59 tienen ruta física. Cambia la sección de `w18` mediante el inspector, exige que las otras 60 rutas sigan exactamente iguales, espera 59 planes V4 y reabre con el mismo almacenamiento. Pasó 3/3 en 41,8 s y en una repetición con captura pasó 3/3 en 43,1 s; 0 errores JavaScript. La edición hasta tener los 59 planes tardó 3286/3019 ms en esas dos pasadas; `esperarPersistencia()` tardó 345/521 ms. El documento serializado quedó en 299 972 bytes. Son mediciones de este host, no un presupuesto de interacción general ni una prueba del coste de mantener 20 snapshots por proyecto.
